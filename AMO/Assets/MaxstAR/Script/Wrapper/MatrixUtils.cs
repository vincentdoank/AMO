...
namespace maxstAR
{
	...
	public class MatrixUtils
	{
		...
		internal static Matrix4x4 GetUnityPose(Matrix4x4 matrix, GameObject camera)
        {
			Matrix4x4 tempPose = matrix;
			Quaternion rotation = camera.transform.rotation;
			Matrix4x4 m = Matrix4x4.TRS(camera.transform.position, rotation, new Vector3(1, 1, 1));
		...
		internal static float[] MatrixToFloat(Matrix4x4 matrix)
		{
			float[] returnMatrix = new float[16];
			returnMatrix[0] = matrix.m00;
			returnMatrix[1] = matrix.m01;
			returnMatrix[2] = matrix.m02;
			returnMatrix[3] = matrix.m03;

			returnMatrix[4] = matrix.m10;
			returnMatrix[5] = matrix.m11;
			returnMatrix[6] = matrix.m12;
			returnMatrix[7] = matrix.m13;

			returnMatrix[8] = matrix.m20;
			returnMatrix[9] = matrix.m21;
			returnMatrix[10] = matrix.m22;
			returnMatrix[11] = matrix.m23;

			returnMatrix[12] = matrix.m30;
			returnMatrix[13] = matrix.m31;
			returnMatrix[14] = matrix.m32;
			returnMatrix[15] = matrix.m33;
