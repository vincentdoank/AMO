...
namespace maxstAR
{
	...
	public class Trackable
    {
		...
		internal Matrix4x4 GetPose(GameObject camera)
		{
			NativeAPI.maxst_Trackable_getPose(cPtr, glPoseMatrix);
		...
		internal Matrix4x4 GetSpaceNRealPose()
		{
			NativeAPI.maxst_Trackable_getPose(cPtr, glPoseMatrix);

			Matrix4x4 targetPose = MatrixUtils.ConvertGLMatrixToUnityMatrix4x4(glPoseMatrix);
			...
			Quaternion rotation = MatrixUtils.QuaternionFromMatrix(convertedMatrix);
		...
		internal Matrix4x4 GetNRealPose()
		{
			NativeAPI.maxst_Trackable_getPose(cPtr, glPoseMatrix);
			...
			rotation = Quaternion.Euler(90, 0, 0);
			Matrix4x4 m = Matrix4x4.TRS(Vector3.zero, rotation, new Vector3(1, 1, 1));
